...
def test_sample_clients_can_be_seated_from_the_server(base_url):
    ...
    # every language this machine can actually run, not a hardcoded pair:
    # whatever clients/ holds gets smoke-tested here
    launchable = sorted(k for k in kinds if k.startswith("client-"))
    assert len(launchable) >= 2, f"too few clients to be a real test: {launchable}"
    for kind in launchable:
...
def test_health_says_whether_the_viewer_is_the_host(base_url):
    """The bracket's join QR belongs on the organiser's screen only, so the
    page asks the server whether this request came from the machine it runs
    on.  The tests talk to it over the loopback, which is the host."""
    status, body = call(base_url, "GET", "/api/health")
    assert status == 200 and body["local"] is True


def test_only_this_machine_counts_as_the_host():
    """The rule itself, for addresses a test cannot dial from."""
    handler = srv.Handler.__new__(srv.Handler)

    class FakeServer:
        local_addresses = {"127.0.0.1", "::1", "192.168.1.187"}

    handler.server = FakeServer()
    for ip, expected in [("127.0.0.1", True),          # localhost
                         ("127.0.0.5", True),          # any loopback
                         ("::1", True),                # IPv6 loopback
                         ("192.168.1.187", True),      # the host via its own LAN address
                         ("::ffff:192.168.1.187", True),   # same, IPv4 over IPv6
                         ("192.168.1.42", False),      # a teammate's laptop
                         ("::ffff:192.168.1.42", False),
                         ("10.0.0.5", False),
                         ("", False)]:
        handler.client_address = (ip, 50000)
        assert srv.Handler.is_local_client(handler) is expected, ip


def test_pause_stops_the_clock_and_the_bots(base_url):
    """Pause is for talking over a position: the clock stops, bots hold still,
    moves are refused, and nobody is charged for the pause."""
    _, game = call(base_url, "POST", "/api/games", {"stones": 200, "cards": 30, "time_limit": 120})
    gid = game["id"]
    _, a = call(base_url, "POST", f"/api/games/{gid}/join", {"name": "A", "seat": 1})
    call(base_url, "POST", f"/api/games/{gid}/bot", {"kind": "greedy", "seat": 2, "delay": 0})

    status, state = call(base_url, "POST", f"/api/games/{gid}/pause")
    assert status == 200 and state["paused"] is True
    clocks = [p["time_remaining"] for p in state["players"]]
    moves = len(state["moves"])

    # a move is refused while paused, and changes nothing
    status, err = call(base_url, "POST", f"/api/games/{gid}/move", {"card": 1}, token=a["token"])
    assert status == 409 and "paused" in err["error"]

    time.sleep(1.2)
    _, state = call(base_url, "GET", f"/api/games/{gid}")
    assert len(state["moves"]) == moves, "a bot moved while paused"
    now = [p["time_remaining"] for p in state["players"]]
    assert max(abs(x - y) for x, y in zip(now, clocks)) < 0.4, (clocks, now)

    status, state = call(base_url, "POST", f"/api/games/{gid}/resume")
    assert status == 200 and state["paused"] is False
    # play works again
    status, _ = call(base_url, "POST", f"/api/games/{gid}/move", {"card": 1}, token=a["token"])
    assert status == 200


def test_pausing_a_game_that_is_not_running_is_harmless(base_url):
    _, game = call(base_url, "POST", "/api/games", {"stones": 10, "cards": 4})
    gid = game["id"]
    status, state = call(base_url, "POST", f"/api/games/{gid}/pause")   # nobody seated yet
    assert status == 200 and state["paused"] is False and state["status"] == "waiting"
    call(base_url, "POST", f"/api/games/{gid}/abort", {"reason": "done"})
    status, state = call(base_url, "POST", f"/api/games/{gid}/pause")   # already over
    assert status == 200 and state["paused"] is False
