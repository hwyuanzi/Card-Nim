...
def choose_card(stones, my_cards, opp_cards)
  return stones if my_cards.include?(stones)              # take the win
  fit = my_cards.select { |c| c <= stones }               # cards that still fit
  return my_cards.first || 1 if fit.empty?                # nothing fits: any card loses
  safe = fit.reject { |c| opp_cards.include?(stones - c) } # do not leave an exact match
  (safe.empty? ? fit : safe).min
end
