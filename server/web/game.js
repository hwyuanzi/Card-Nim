...
(function () {
  ...
  // advancing one move every REPLAY_SPEED ms.  Nothing is sent to the server.
  // One speed, so a replay looks the same wherever it is watched.
  const REPLAY_SPEED = 2000;
  let replay = { active: false, index: 0, playing: false, timer: null };
  ...
  function replayPlay() {
    ...
    replay.timer = setInterval(replayTick, REPLAY_SPEED);
  ...
  /* Purpose: the secret of the tournament entrant this browser joined as, if
     the game belongs to a bracket.  Sent when sitting down so the reserved
     seat opens even if two entrants share a name. */
  function entrantToken() {
    if (!state || !state.tournament) return null;
    try {
      const rec = JSON.parse(localStorage.getItem("cardnim_entrant_" + state.tournament.id) || "null");
      return rec && typeof rec.token === "string" ? rec.token : null;
    } catch (e) { return null; }
  }

  async function api(method, path, body, seat, extra) {
    let res;
    try {
      res = await fetch(path, {
        method,
        headers: authHeaders(seat, Object.assign({ "Content-Type": "application/json" }, extra || {})),
  ...
  async function join(seat, name, avatar) {
    const extra = {};
    const claim = entrantToken();
    if (claim && !held(seat)) extra["X-Token"] = claim;
    const payload = await api("POST", `/api/games/${gameId}/join`, { name, seat, avatar }, seat, extra);
  ...
  function render() {
    ...
    const nb = $("nav-bracket");
    if (nb) { nb.hidden = !s.tournament; if (s.tournament) nb.href = "/tournament/" + s.tournament.id; }
    const cover = $("cover");
  ...
  function renderBanner() {
    ...
    if (replay.active) {
      ...
      actions = `<button class="btn small" id="rp-restart" title="Restart">|&lt;</button>` +
        ...
        `<button class="btn small" id="rp-exit">Exit replay</button>`;
    } else if (s.status === "waiting") {
      const open = s.players.filter((p) => !p.occupied).length;
      const kept = s.players.filter((p) => !p.occupied && p.reserved).map((p) => esc(p.name));
      text = kept.length ? `Waiting for ${kept.join(" and ")} to sit down.`
        : open === 2 ? "Waiting for two players to sit down." : "Waiting for one more player.";
      ...
      actions = (s.moves.length ? '<button class="btn small" id="replay-btn">Replay</button>' : "") +
                (s.tournament ? `<a class="btn gold small" href="/tournament/${esc(s.tournament.id)}">Back to the bracket</a>`
  ...
  function renderPlayer(seat) {
    ...
    const reservedOpen = s.status === "waiting" && !p.occupied && p.reserved;
    const joinMode = s.status === "waiting" && !p.occupied && !p.reserved;
    ...
    if (joinMode) {
      ...
      nameEl.innerHTML = p.occupied
        ...
        : reservedOpen ? esc(p.name) + ' <span class="botbadge">reserved</span>'
      ...
      if (avatar) {
        ...
        if ((p.occupied || reservedOpen) && p.avatar) {
      ...
      if (pop) pop.remove();
      if (cardsLeft && reservedOpen) {
        // the bracket put this entrant here; one click sits them down (no name to type)
        cardsLeft.innerHTML = `<button class="btn black small" type="button" data-claim${joining[seat] ? " disabled" : ""}>Sit here as ${esc(p.name)}</button>` +
          `<span class="error" data-err></span>`;
        const btn = cardsLeft.querySelector("[data-claim]");
        const err = cardsLeft.querySelector("[data-err]");
        btn.addEventListener("click", () => {
          if (joining[seat]) return;
          joining[seat] = true;
          btn.disabled = true;
          join(seat, p.name, p.avatar).then(() => { joining[seat] = false; })
            .catch((e) => { joining[seat] = false; err.textContent = e.message; btn.disabled = false; });
        });
      } else if (cardsLeft) {
        cardsLeft.textContent = p.occupied ? `${p.cards.length} of ${s.num_cards} cards` : "";
      }
    }
    ...
    if (replay.active) {
      ...
      note.textContent = "";
    } else if (reservedOpen) {
      note.textContent = `Reserved for ${p.name} by the tournament. Waiting for them to sit down.`;
  ...
  function renderWinner() {
    ...
    if (celebrated !== key) {
      celebrated = key;
      if (w) window.cardnimFireworks($("fireworks"), 4200);
    }
  }
