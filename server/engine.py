...
class Seat:
    ...
    reservation:     set by a tournament: the seat is kept for one named
                     entrant and nobody else may sit there.  The value is the
                     entrant's secret, which may be sent as the token when
                     joining; joining with the reserved name also works.
                     None for an ordinary seat.
    """
...
class Game:
    ...
    def __init__(
        ...
        self.tournament: Optional[dict] = None   # {"id", "label", "round", "match"} when part of a bracket
        self._clock = clock
    ...
    def reserve(self, seat_number: int, name: str, avatar: int, secret: str) -> Seat:
        """Purpose: keep a seat for one tournament entrant.
        Inputs:  seat number, the entrant's name and avatar, and the entrant's
                 secret (sent as the token when they sit down).
        Outputs: the Seat.
        Side effects: the seat shows the name but stays unoccupied; the game
                      does not start until the entrant joins.  Bumps version.
        Raises GameError if the seat is taken or the game is not waiting."""
        if self.status != STATUS_WAITING:
            raise GameError("seats can only be reserved before the game starts")
        seat = self.seats[seat_number]
        if seat.occupied:
            raise GameError(f"seat {seat_number} is already taken by {seat.name}")
        seat.name = clean_text(name, 40) or "anonymous"
        seat.avatar = int(avatar) if avatar else self._pick_avatar(seat.name, seat_number)
        seat.reservation = secret
        self._touch()
        return seat

    def _seat_to_take(self, name: str, seat_number: Optional[int], claim: Optional[str]) -> Seat:
        """Purpose: decide which seat a joiner gets, honouring reservations.
        Inputs:  cleaned name, seat preference (or None), and the token the
                 joiner sent (or None).
        Outputs: an unoccupied Seat.
        Raises GameError when the table is full, the requested seat is taken,
        or the requested seat is reserved for somebody else."""
        def may_take(seat: Seat) -> bool:
            if seat.occupied:
                return False
            if not seat.reserved:
                return True
            if claim and secrets.compare_digest(seat.reservation, claim):
                return True
            return seat.name.lower() == name.lower()

        if seat_number is None:
            # a seat kept for this joiner comes first, then any open seat
            for s in (1, 2):
                if self.seats[s].reserved and may_take(self.seats[s]):
                    return self.seats[s]
            for s in (1, 2):
                if may_take(self.seats[s]):
                    return self.seats[s]
            if all(s.occupied for s in self.seats.values()):
                raise GameError("both seats are taken")
            kept = [s.name for s in self.seats.values() if s.reserved and not s.occupied]
            raise GameError(f"the open seat is reserved for {' and '.join(kept)}")
        if seat_number not in (1, 2):
            raise GameError("seat must be 1 or 2")
        seat = self.seats[seat_number]
        if seat.occupied:
            raise GameError(f"seat {seat_number} is already taken by {seat.name}")
        if not may_take(seat):
            raise GameError(f"seat {seat_number} is reserved for {seat.name}")
        return seat

    def join(self, name: str, seat_number: Optional[int] = None, now: Optional[float] = None,
             avatar: Optional[int] = None, is_bot: bool = False, claim: Optional[str] = None) -> Seat:
        ...
        Inputs:  display name; optional seat preference (1 or 2); current time;
                 ...
                 the same face, avoiding the opponent's.  `claim` is the token
                 the joiner sent, if any: a reserved seat opens for the token it
                 was reserved with (the token then also works as the seat
                 token) or for a joiner giving the reserved name.
        ...
        Raises GameError if the game is over, the requested seat is taken or
        reserved for somebody else, the table is full, or the avatar number is
        out of range."""
        ...
        seat = self._seat_to_take(name, seat_number, claim)
        seat_number = seat.number
        if seat.reserved:
            # the bracket decided the name and the face; the entrant's secret is the seat token
            seat.token = seat.reservation
        else:
            seat.name = name
    ...
    def play(self, seat_number: int, card: int, now: Optional[float] = None) -> Move:
        ...
        if self.paused:
            raise IllegalMove("the game is paused")
        if seat_number != self.turn:
            raise IllegalMove("it is not your turn")
        seat = self.seats[seat_number]
        try:
            # int(2.7) is 2, which would quietly play a card nobody asked for,
            # so a value that is not already whole is refused outright
            if isinstance(card, float) and not card.is_integer():
                raise ValueError
        ...
        # This turn is now paid for.  Clearing the start stops _finish() from
        # charging it a second time when this move ends the game; the branch
        # below that passes the turn on sets it again for the next player.
    ...
    def resume(self, now: Optional[float] = None) -> None:
        """Purpose: start play again; the mover's clock runs from now, so the
        pause costs nobody anything.  Bumps version."""
    ...
    def check_timeout(self, now: Optional[float] = None) -> bool:
        ...
        now = self._clock() if now is None else now
        if self.status != STATUS_PLAYING or self.paused:
            return False                    # a paused clock cannot run out
    ...
    def leave(self, seat_number: int) -> None:
        ...
        if self.status == STATUS_WAITING:
            seat.token = None
            seat.is_bot = False
            if not seat.reserved:          # a tournament seat keeps its name and stays reserved
                seat.name = ""
                seat.avatar = 0
            self._touch()
        else:
    ...
    def summary(self, now: Optional[float] = None, hands: bool = False) -> dict:
        """Purpose: the short form used by the lobby list.
        Inputs: `hands` adds each seat's remaining cards, which the bracket
        needs to draw a match as a real table.  The lobby leaves it off: it
        lists every game, and the cards would be the bulk of the response.
        Outputs: a JSON-serialisable dict."""
        now = self._clock() if now is None else now
        extra = {"hands": [sorted(self.seats[1].cards), sorted(self.seats[2].cards)],
                 "last_move": ({"seat": self.moves[-1].seat, "card": self.moves[-1].card}
                               if self.moves else None)} if hands else {}
        return {
            **extra,
            "id": self.id,
            "label": self.label,
            ...
            "occupied": [self.seats[1].occupied, self.seats[2].occupied],
            # enough for the bracket to draw each match as a small table
            "cards_left": [len(self.seats[1].cards), len(self.seats[2].cards)],
            "time_remaining": [round(self.time_remaining(1, now), 3),
